...
const SCOPES: ReadonlySet<Scope> = new Set(['all', 'invoices', 'customers', 'articles', 'expenses', 'deals'])
...
export default defineEventHandler(async (event): Promise<SearchPayload> => {
  ...
  const empty: SearchPayload = { invoices: [], customers: [], articles: [], expenses: [], deals: [] }
  ...
  if (want('deals')) {
    out.deals = db
      .prepare(
        `SELECT d.id, d.name, d.direction, d.stage, c.name AS customer_name
         FROM deals d
         LEFT JOIN customers c ON c.id = d.customer_id
         WHERE d.name LIKE ? ESCAPE '\\' OR d.label LIKE ? ESCAPE '\\' OR c.name LIKE ? ESCAPE '\\'
