...
const SALES_STAGES: readonly Stage[] = [
  'lead',
  'contacted',
  'proposal',
  'won',
...
export default defineEventHandler(async (event) => {
  ...
  const stage: Stage =
    body.stage && validStages.includes(body.stage) ? body.stage : (fallback as Stage)
  ...
  const budgetType: BudgetType =
    body.budget_type && BUDGET_TYPES.has(body.budget_type) ? body.budget_type : 'fixed'
