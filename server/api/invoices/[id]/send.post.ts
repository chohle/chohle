...
// HTML shell, plaintext, and the contact footer all come from the shared
// branded template (server/utils/emailTemplate.ts) so every outbound email
// looks identical. buildBrandedEmail is auto-imported.
...
export default defineEventHandler(async (event) => {
  ...
  const sender = db
    .prepare(
      'SELECT name, email, phone, website, mwst, logo_path, iban, street, zip, city FROM sender WHERE id = 1'
    )
    .get() as
    | {
        name: string
  ...
  if (!sender || !isIBANValid(iban)) {
    throw createError({
      statusCode: 422,
      statusMessage: 'Set a valid IBAN in Billing to send the invoice'
    })
  }
  ...
  db.prepare(
    "UPDATE invoices SET status = 'sent', paid_at = NULL, total_rappen = NULL WHERE id = ?"
  ).run(id)
  return { ok: true }
})
