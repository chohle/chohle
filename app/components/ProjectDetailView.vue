...
// --- Tabs ---
// The page used to stack every section in one long scroll. Splitting it into
// tabs gives the conversation the full width it needs to read as a thread, and
// keeps quotes/invoices one click away instead of far down the page.
type DetailTab = 'overview' | 'conversation' | 'quotes' | 'invoices'
const activeTab = ref<DetailTab>('overview')
const isSales = computed(() => project.value?.direction === 'sales')
const tabOptions = computed(() => {
  const list = [
    { value: 'overview', label: t('pipeline.detail.tabOverview') },
    { value: 'conversation', label: t('pipeline.detail.timeline') }
  ]
  if (isSales.value) {
    list.push({ value: 'quotes', label: t('pipeline.detail.tabQuotes') })
    list.push({ value: 'invoices', label: t('pipeline.detail.tabInvoices') })
  }
  return list
})
...
// Who each bubble is from: "You" for outbound, the contact's name for inbound.
const contactName = computed(
  () => project.value?.customer_name || project.value?.name || t('pipeline.detail.noContact')
)
function senderLabel(ev: ProjectEmail): string {
  return ev.direction === 'outbound' ? t('pipeline.detail.you') : contactName.value
}
...
// Inbound HTML is sanitized via the shared sanitizeHtml util before binding
// through v-html. The Compose slideover handles both new emails and
...
// Reply to a specific message: open the Compose slideover addressed to the
// other party on that message, with a "Re:" subject and a blank body.
...
// --- Paste a reply (log a received message) ---
...
// Deep link from the Conversations page (?reply=<emailId>) opens the Compose
// slideover pre-filled as a reply to that message once the thread has loaded.
...
<template>
  <div v-if="project" class="page-deal-detail">
    ...
    <div class="page-deal-detail__tabs">
      <UiSegmentedControl
        :model-value="activeTab"
        :options="tabOptions"
        :aria-label="$t('pipeline.detail.tabOverview')"
        @update:model-value="(v: string) => (activeTab = v as DetailTab)"
      />
    </div>
    ...
    <div
      ...
      class="page-deal-detail__panel page-deal-detail__overview"
    >
      <UiCard v-if="project.direction === 'sales' && project.budget_rappen > 0">
    ...
    <!-- Conversation -->
    <div v-show="activeTab === 'conversation'" class="page-deal-detail__panel">
      <UiCard>
          ...
          <div class="chat">
            <section v-for="g in emailGroups" :key="g.key" class="email-group">
              <header class="email-group__head">
                <h4 class="email-group__subject">
                  {{ g.subject || $t('conversations.noSubject') }}
                </h4>
              ...
              <ul class="chat-thread">
                ...
                >
                  <div class="chat-msg__meta mono">
                    <span class="chat-msg__who">{{ senderLabel(ev) }}</span>
                    <span>{{ fmtTimestamp(ev.sent_at) }}</span>
                  </div>
                  <div class="chat-msg__bubble">
                    <div v-if="ev.body_html" v-html="sanitizeHtml(ev.body_html)" />
                    <pre v-else-if="ev.body_text" class="chat-msg__bubble--text">{{
                      ev.body_text
                    }}</pre>
                  </div>
                  <button
    ...
    <!-- Quotes -->
    <div
      v-show="activeTab === 'quotes' && project.direction === 'sales'"
      class="page-deal-detail__panel"
    >
      <UiCard>
        <div class="page-deal-detail__timeline-head">
    ...
    <!-- Invoices -->
    <div
      v-show="activeTab === 'invoices' && project.direction === 'sales'"
      class="page-deal-detail__panel"
    >
      <UiCard>
        <div class="page-deal-detail__timeline-head">
