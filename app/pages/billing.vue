...
interface Sender {
  ...
  iban: string; uid: string; mwst: string; hr_number: string
...
const logoSrc = computed(() => data.value?.logo_path ? `/api/sender/logo?v=${data.value.logo_path}` : null)

const form = reactive({
  type: d.type, name: d.name, street: d.street, zip: d.zip, city: d.city, country: d.country,
  email: d.email, phone: d.phone, website: d.website,
  iban: d.iban, uid: d.uid, mwst: d.mwst, hrNumber: d.hr_number,
  foundingYear: d.founding_year ?? undefined as number | undefined,
  vatRegistered: !!d.vat_registered
})

watch(() => form.type, (t) => { form.vatRegistered = t === 'company' })
