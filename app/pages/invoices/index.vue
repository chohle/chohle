...
<template>
  <div class="page-invoices">
    <UiPageHead
      :crumb="`${$t('nav.workspace')} / ${$t('nav.invoices')}`"
      :title="$t('nav.invoices')"
      :subtitle="$t('invoices.allSubtitle')"
    >
      <template #actions>
    ...
    <UiKpiRow>
      <UiKpiCell
        :label="$t('invoices.kpiPaid')"
        currency="CHF"
        :value="chf(paidTotal)"
        :delta="$t('invoices.deltaCount', { n: countBy('paid') })"
      />
      <UiKpiCell
        :label="$t('invoices.kpiSentAwaiting')"
        currency="CHF"
        :value="chf(sentTotal)"
        :delta="$t('invoices.deltaCount', { n: countBy('sent') })"
      />
      <UiKpiCell
        :label="$t('invoices.kpiDrafts')"
        currency="CHF"
        :value="chf(draftTotal)"
        :delta="$t('invoices.deltaDrafts', { n: countBy('draft') })"
      />
      <UiKpiCell
        :label="$t('invoices.kpiTotal')"
        currency="CHF"
        :value="chf(paidTotal + sentTotal + draftTotal)"
        :delta="$t('invoices.deltaAll', { n: invoices.length })"
      />
    </UiKpiRow>
          ...
          <tbody>
            ...
            >
              ...
              <td class="page-invoices__muted">
                <span v-if="inv.project_name" class="page-invoices__project">{{
                  inv.project_name
                }}</span>
              ...
              <td>
                <UiOutlinedChip :status="inv.status">{{
                  $t(`status.${inv.status}`)
                }}</UiOutlinedChip>
              </td>
