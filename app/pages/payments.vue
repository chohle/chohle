...
interface Payload { year: number; rows: PaymentRow[]; total: number }
...
const grouped = computed(() => {
  ...
  return [...groups.entries()].map(([ym, rows]) => ({
    ym, rows, total: rows.reduce((s, r) => s + r.amount_rappen, 0)
  }))
})

const invoiceTotal = computed(() =>
  (data.value?.rows ?? []).filter(r => r.kind === 'invoice').reduce((s, r) => s + r.amount_rappen, 0))
const salaryTotal = computed(() =>
  (data.value?.rows ?? []).filter(r => r.kind === 'salary').reduce((s, r) => s + r.amount_rappen, 0))
</script>
        ...
        <tbody>
          <template v-for="g in grouped" :key="g.ym">
            ...
            <tr v-for="r in g.rows" :key="`${r.kind}-${r.id}`" class="row">
              ...
              <td>
                <div class="src">
                  <UIcon :name="r.kind === 'invoice' ? 'i-lucide-file-text' : 'i-lucide-briefcase'" class="size-3.5" />
                  <div>
                    <component :is="r.link ? 'NuxtLink' : 'span'" :to="r.link" class="src-name">{{ r.label }}</component>
