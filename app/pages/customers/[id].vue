...
const { data: projects } = await useFetch<ProjectRow[]>(`/api/customers/${id}/projects`, {
  default: () => []
})
...
async function onProjectResolved(projectId: number) {
  ...
  try {
    const { id: invoiceId } = await $fetch<{ id: number }>(`/api/projects/${projectId}/invoices`, {
      method: 'POST'
    })
    ...
    pickerOpen.value = true
    const msg =
      (err as { statusMessage?: string }).statusMessage ?? t('customers.invoiceCreateFailed')
...
<template>
  <div v-if="customer" class="page-customer-detail">
        ...
        <table class="ed-table">
          ...
          <tbody>
            ...
            >
              ...
              <td class="right mono">
                {{ p.budget_rappen > 0 ? `CHF ${chf(p.budget_rappen)}` : '—' }}
              </td>
              <td class="right mono">
                <span v-if="p.invoice_count"
                  >CHF {{ chf(p.invoiced_rappen) }}
                  <span class="page-customer-detail__project-count"
                    >({{ p.invoice_count }})</span
                  ></span
                >
                <span v-else>—</span>
              </td>
    ...
    <UModal v-model:open="pickerOpen" :title="$t('customers.newInvoice')">
      <template #body>
        <p class="page-customer-detail__picker-hint">
          {{ $t('customers.invoiceFromProjectHint') }}
        </p>
        <ProjectPicker
