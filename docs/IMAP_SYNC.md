...
specific setup. For the shared concepts (BATZE_SECRET, threading,
...
2. **Settings → Privacy & Security → App passwords → New app
   password**. Name it `batze`, give it **Mail (IMAP / POP / SMTP)**
   access.
...
5. Click **Test and connect**. batze opens a real IMAP session and
...
Settings → Mail sync → row → **Disconnect**. batze forgets the
...
For non IMAP troubleshooting (BATZE_SECRET, threading not matching,
